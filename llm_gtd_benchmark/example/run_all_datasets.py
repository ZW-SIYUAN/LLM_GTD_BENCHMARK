...
GraDe Experiment Runner — 6 datasets (excluding us_location)
  ...
  col_order  : fixed (specified per dataset)
...
DATASETS = {
    ...
    "house": {
        ...
        "sample_k":   50,      # reduced: 10col×150tok KV-cache 会大
        "drop_cols":  [],
    },
...
def make_logger(name: str) -> logging.Logger:
    log_file = f"{name}_experiment.log"
...
# ── per-dataset runner ────────────────────────────────────────────────────────
def run_dataset(name: str, cfg: dict, test_mode: bool = False):
    logger = make_logger(name)
    logger.info("=" * 60)
    logger.info("GraDe · %s · %s + LoRA%s", name, LLM, "  [TEST MODE]" if test_mode else "")
    logger.info("=" * 60)

    # ── paths
    train_path     = os.path.join("data", name, "train.csv")
    checkpoint_dir = f"checkpoints_{name}_test" if test_mode else f"checkpoints_{name}"
    output_path    = f"synthetic_{name}_test.csv" if test_mode else f"synthetic_{name}.csv"
    ...
    # ── train (test mode: only 3 optimizer steps via max_steps)
    logger.info("Training  |  %d samples  |  epochs=%d%s",
                len(train_df), epochs, "  max_steps=3" if test_mode else "")
    t0 = time.time()

    fit_kwargs = dict(conditional_col=cfg["start_col"], fd_list=cfg["fd_list"])
...
def main():
    ...
    for name in targets:
        try:
            run_dataset(name, DATASETS[name], test_mode=args.test)
